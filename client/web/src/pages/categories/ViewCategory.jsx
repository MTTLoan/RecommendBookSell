...
const ViewCategory = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const cat = (cats || []).find(c => String(c.id || c._id) === String(id));
      setCategory(cat);
      const filtered = (books || []).filter(b => String(b.categoryId) === String(cat.id || cat._id));
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (prod) => (
        <img
          src={prod.images?.[0]?.url || prod.images?.[0] || 'https://via.placeholder.com/40x40?text=No+Img'}
          alt={prod.name}
      ...
      <main className="dashboard-content">
        <div className="product-header">
          ...
          <p className="product-subtitle">
            <span className="subtitle" style={{ cursor: 'pointer' }} onClick={() => navigate('/categories')}>Danh mục</span>
